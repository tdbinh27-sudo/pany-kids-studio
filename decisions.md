...
### D-043 Phase 2: "Đoán Từ Vựng" (Vocabulary Wordle) added to Practice Corner ✅ SHIPPED
Self-contained Wordle-style vocab game (components/VocabWordle.tsx, ~230 lines) rendered inside PracticeTab. Educational twist: shows the Vietnamese meaning as the clue → child guesses the 5-letter English word (34-word A2/KET bank, VI+EN clues). Standard correct/present/absent scoring with duplicate-letter handling, on-screen QWERTY + physical keyboard, bilingual, tablet-friendly. Concept ported from cwackerfuss/react-wordle (MIT), code written fresh. Absent-state color darkened (#94a3b8) per QA for kid readability. Skipped PhET-geometry-into-STEM: GeoGebra 3D/2D (D-043 P1) already covers geometry better + PhET lacks a strong geometry sim + STEM is DB-driven (data edit, not code). tsc + build + Playwright QA (typed a word, verified green/grey coloring + win banner) pass, 0 errors.
